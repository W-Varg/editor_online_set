      ...
      <div class="demo-info">
        ...
        <div class="demo-users">
          <span
            ...
            :key="u"
            class="demo-user"
            @click="username = u; password = 'Admin123@'"
          >
            {{ u }}
          </span>
        </div>
        <p class="demo-hint">Click en el usuario → completa los campos con contraseña <code>Admin123@</code></p>
      </div>
    </div>
  </div>
</template>
...
h1 { margin: 0; font-size: 1.5rem; color: #1a1a2e; }
.subtitle { color: #666; font-size: 0.85rem; margin: 0.3rem 0 1.5rem; }
...
.demo-info { margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid #eee; }
.demo-label { font-size: 0.8rem; color: #888; font-weight: 600; margin-bottom: 0.5rem; }
.demo-users { display: flex; gap: 0.4rem; justify-content: center; flex-wrap: wrap; }
.demo-user {
  padding: 0.3rem 0.6rem; background: #f0f4ff; color: #2563eb;
  border-radius: 6px; font-size: 0.8rem; font-weight: 500;
  cursor: pointer; transition: background 0.15s;
}
.demo-user:hover { background: #dbeafe; }
.demo-hint { font-size: 0.75rem; color: #999; margin-top: 0.5rem; }
code { background: #f1f5f9; padding: 0.1rem 0.3rem; border-radius: 3px; font-size: 0.75rem; }
</style>
